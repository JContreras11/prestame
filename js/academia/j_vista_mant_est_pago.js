$(function(){
  ...
  function jalar_data() {
    ...
    var nrox=0;
    var otablex = {};
    $.getJSON(Gruta+'academia/Mantenimiento/mant_est_pago_listar', function(data) {
      ...
      $.each(data, function(k,item){
        ...
        <td  >${item.detalle_est_pago} </td>
         <td class="ver_registro" idr="${item.ide_es_pago}" ><i class="fa fa-search"></i> </td>
         <td class="edi_registro" idr="${item.ide_es_pago}"><i class="fa fa-pencil"></i> </td>
         <td class="bor_registro" idr="${item.ide_es_pago}"><i class="fa fa-trash-o"></i> </td>
         </tr> `;
    ...
    function pjaxPageLoad(){
      ...
      $(".form1").on('submit', function(e){  // Cambio de id a clase
        ...
        switch(tipo_accion) {
            case "nuevo":
                destino = "mant_est_pago_ins";
                break;
            case "edita":
                destino = "mant_est_pago_upd";
                break;
            case "borra":
                destino = "mant_est_pago_del";
                break;
        }
  ...
  $('#myModalver #myModaleditaLabel').html('Estado');
    $('#accion').val('edita');
    var det_registro='';
    $.getJSON(Gruta+'academia/Mantenimiento/mant_est_pago_listar/'+idr, function(data) { // Inicio JSON
      $.each(data, function(k,item){
        det_registro+=`<tr><th>Id: </th> <td>${item.ide_es_pago}</td></tr>
        <tr><th>Tipo Movimiento </th><td>${item.detalle_est_pago}</td></tr>`;
      });
    ...
    $.getJSON(Gruta+'academia/Mantenimiento/mant_est_pago_listar/'+idr, function(data) { // Inicio JSON
      $.each(data, function(k,item){
        $('#inp_text1').val(item.ide_es_pago);
        $('#inp_text2').val(item.detalle_est_pago);
        });
      $('#myModaledita').modal('show')
    });
  });
    ...
    $("#datatable-table").on('click','.bor_registro',function() {
      ...
      $.getJSON(Gruta+'academia/Mantenimiento/mant_est_pago_listar/'+idr, function(data) { // Inicio JSON
        $.each(data, function(k,item){
          $('#nom').val(item.detalle_est_pago);
          $('#ide').val(item.ide_es_pago);
          });
